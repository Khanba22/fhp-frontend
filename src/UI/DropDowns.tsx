          ...
          <div className="py-1">
            {options.map((option) => (
              ...
              >
                ...
                <span className={value === option ? "ml-0" : "ml-6"}>
                  {option}
                </span>
              </button>
            ))}
...
export function MultiSelectDropdown({
  ...
  const colorArr = ['bg-blue-100 text-blue-700', 'bg-green-100 text-green-700', 'bg-red-100 text-red-700', 'bg-yellow-100 text-yellow-700', 'bg-purple-100 text-purple-700', 'bg-orange-100 text-orange-700', 'bg-pink-100 text-pink-700', 'bg-gray-100 text-gray-700'];
        ...
        <div className="flex flex-wrap gap-1 flex-1">
          {value.length === 0 ? (
            ...
            value.map((item,index) => {
                if (index < 3) {
                    return (
                        <span key={item} className={`inline-flex items-center px-2 py-1 text-xs rounded-full ${colorArr[index]}`}>
                            {item}
                        </span>
                    )
                }
